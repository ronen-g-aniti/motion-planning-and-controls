...
class PathPlanner(Drone):
	...
	def plan_path(self, target):
		...
		self.current_pos = np.array([self.local_position[0], self.local_position[1], self.local_position[2] * -1.0])
		self.path = coarse_path(self.current_pos, target, self.states, self.states_kd, self.obstacles) # A star with path shortening
		if len(self.path) > 0:
...
class Follower:
	def __init__(self, start, goal, states, states_kd, obstacles, obstacles_kd, leader):
		...
		self.obstacles = obstacles
		self.obstacles_kd = obstacles_kd
		self.path = coarse_path(start, goal, states, states_kd, obstacles)
		self.target = self.path.pop(0)
	...
	def step_forward(self):
		...
		if self.count % 50 == 0 and self.count != 0 and np.linalg.norm(self.leader.current_pos - self.current_pos) > 50.0:
			self.path = coarse_path(self.current_pos, self.leader.current_pos, states, states_kd, self.obstacles)
			self.target = self.path.pop(0)
...
def coarse_path(start, goal, states, states_kd, obstacles):
...
def shorten(path):
	"""Removes collinear waypoints from the path and returns a shortened path"""
	tol = 1e-6
...
def vis(position_history_target, position_history_drone, obstacles):
	...
	def animate(frame):
		...
		if frame % skip_frames == 0:
			k = frame // skip_frames  # Index of the point to plot on this frame
